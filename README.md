...
- `LiveGenerationContractTest` — generate → контракт по **RAW** → `Judge` (вердикт в лог, `REJECTED` live не валит). `mixed-layer`, `hallucinate-*` только с `-Dred=true`.
...
Красный демо офлайн: `@Step` на методе `*Tests` или `fillAndSubmitForm` в `login-wrong-password-e2e` фикстуре. Pack: `test-negative` в API `expect.rag` или «неуспешный» в `index:` у `po-fluent`. Live `-Dred`: оба слоя вместо `Отказ.`, эхо `Invalid password`, over-refuse селектора.
