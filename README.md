...
**Mill занятия:** [START.md](START.md) — без модели, live смоук, красный 7b на `-Dred`.
...
- `LiveGenerationContractTest` — generate → контракт по **RAW** → `Judge` (вердикт в лог, `REJECTED` live не валит). `login-401-api`, `mixed-layer`, `hallucinate-*` только с `-Dred=true`.
...
Красный демо офлайн: `@Step` на методе `*Tests` или `fillAndSubmitForm` в `login-wrong-password-e2e` фикстуре. Pack: `test-negative` в API `expect.rag` или «неуспешный» в `index:` у `po-fluent`. Live `-Dred`: subset `RAG:` на api, оба слоя вместо `Отказ.`, эхо `Invalid password`, over-refuse селектора.
